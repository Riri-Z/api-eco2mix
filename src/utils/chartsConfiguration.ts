import { compareAsc, subWeeks, format } from 'date-fns';
...
export const EXPORT_MENU_CHARTS_FILTERED = ['viewFullscreen', 'printChart'];

function timeStampTotimeStampPlusTwo(timeStamp: number) {
...
function isRangeLongerThanTwoWeeks(startDate: string, endDate: string) {
...
export default function dataProcessing(values: any, startDate: string, endDate: string) {
  ...
  const seriesElictricityProduction = [
    {
      name: 'Fioul',
      data: values.map((item: IEco2mix) => [timeStampTotimeStampPlusTwo(item.timeStamp), item.fioul])
    },
    {
      name: 'Charbon',
      data: values.map((item: IEco2mix) => [timeStampTotimeStampPlusTwo(item.timeStamp), item.charbon])
    },
    {
      name: 'Gaz',
    ...
    {
      name: 'Nucleaire',
      data: values.map((item: IEco2mix) => [timeStampTotimeStampPlusTwo(item.timeStamp), item.nucleaire])
    },
    {
      name: 'Eolien',
      data: values.map((item: IEco2mix) => [timeStampTotimeStampPlusTwo(item.timeStamp), item.eolien])
    },
    {
      name: 'Solaire',
      data: values.map((item: IEco2mix) => [timeStampTotimeStampPlusTwo(item.timeStamp), item.solaire])
    },
    {
      name: 'Hydraulique',
      data: values.map((item: IEco2mix) => [timeStampTotimeStampPlusTwo(item.timeStamp), item.hydraulique])
    },
    {
      name: 'Pompage',
      data: values.map((item: IEco2mix) => [timeStampTotimeStampPlusTwo(item.timeStamp), item.pompage])
    },
    {
      name: 'Bioenergies',
      data: values.map((item: IEco2mix) => [timeStampTotimeStampPlusTwo(item.timeStamp), item.bioenergies])
    }
  ];
  ...
  const seriesElectricityConsumption = [
    {
      name: 'Consommation',
      data: values.map((item: IEco2mix) => [timeStampTotimeStampPlusTwo(item.timeStamp), item.consommation])
    },
    {
      name: 'Prevision_j1',
      data: values.map((item: IEco2mix) => [timeStampTotimeStampPlusTwo(item.timeStamp), item.prevision_j1])
    },
    {
      name: 'Prevision_j',
      data: values.map((item: IEco2mix) => [timeStampTotimeStampPlusTwo(item.timeStamp), item.prevision_j])
    }
  ];
  ...
  const seriesCo2Rate = [
    {
      ...
      data: values.map((item: IEco2mix) => [timeStampTotimeStampPlusTwo(item.timeStamp), item.taux_co2])
    }
  ];
  ...
  values.forEach((item: IEco2mix) => {
    ...
    for (const key in tradeProperties) {
      if (key !== 'date' && key !== 'date_heure' && tradeProperties[key as keyof typeof tradeProperties] !== null) {
        const value =
